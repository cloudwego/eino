...
type enhancedInvokableTool struct {
	info *schema.ToolInfo
	fn   func(ctx context.Context, input *tool.ToolCallInfo) (*schema.ToolResult, error)
}
...
func (e *enhancedInvokableTool) InvokableRun(ctx context.Context, toolCallInfo *tool.ToolCallInfo, _ ...tool.Option) (*schema.ToolResult, error) {
	return e.fn(ctx, toolCallInfo)
}

type enhancedStreamableTool struct {
	info *schema.ToolInfo
	fn   func(ctx context.Context, input *tool.ToolCallInfo) (*schema.StreamReader[*schema.ToolResult], error)
}
...
func (e *enhancedStreamableTool) StreamableRun(ctx context.Context, toolCallInfo *tool.ToolCallInfo, _ ...tool.Option) (*schema.StreamReader[*schema.ToolResult], error) {
	return e.fn(ctx, toolCallInfo)
}
		...
		fn: func(ctx context.Context, input *tool.ToolCallInfo) (*schema.ToolResult, error) {
			return &schema.ToolResult{
				Content: []schema.ToolOutputPart{
					{Text: "invokable result: " + input.TextArguments},
				},
			}, nil
		},
	}
	...
	enhancedStreamable := &enhancedStreamableTool{
		...
		fn: func(ctx context.Context, input *tool.ToolCallInfo) (*schema.StreamReader[*schema.ToolResult], error) {
			results := []*schema.ToolResult{
				{Content: []schema.ToolOutputPart{{Text: "stream part 1: " + input.TextArguments}}},
				{Content: []schema.ToolOutputPart{{Text: " stream part 2"}}},
			}
...
func TestEnhancedToolConversion(t *testing.T) {
	...
	enhancedInvokable := &enhancedInvokableTool{
		...
		fn: func(ctx context.Context, input *tool.ToolCallInfo) (*schema.ToolResult, error) {
			return &schema.ToolResult{
				Content: []schema.ToolOutputPart{
					{Text: "enhanced: " + input.TextArguments},
				},
			}, nil
		},
	}
...
func TestEnhancedToolMiddleware(t *testing.T) {
	...
	t.Run("enhanced streamable middleware", func(t *testing.T) {
		...
		assert.True(t, streamableMiddlewareCalled)
	})
}
...
func TestEnhancedToolPriority(t *testing.T) {
	...
	t.Run("enhanced tool is used when available", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Len(t, output, 1)
		assert.Contains(t, output[0].Content, "enhanced result")
	})
}
