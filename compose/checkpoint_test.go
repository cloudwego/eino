...
func TestSimpleCheckPoint(t *testing.T) {
	RegisterType("test_struct", &testStruct{})
	...
	_, err = r.Invoke(ctx, "start", WithCheckPointID("1", func(ctx context.Context, path []string, state any) error {
	...
	result, err := r.Invoke(ctx, "start", WithCheckPointID("1", func(ctx context.Context, path []string, state any) error {
		assert.Equal(t, 0, len(path))
	...
	_, err = r.Stream(ctx, "start", WithCheckPointID("2", func(ctx context.Context, path []string, state any) error {
		assert.Equal(t, 0, len(path))
		state = "state"
		return nil
	}))
	...
	streamResult, err := r.Stream(ctx, "start", WithCheckPointID("2", func(ctx context.Context, path []string, state any) error {
		assert.Equal(t, 0, len(path))
		state.(*testStruct).A = "state"
		return nil
	}))
...
func TestSubGraph(t *testing.T) {
	...
	_, err = r.Stream(ctx, "start", WithCheckPointID("2", nil))
	assert.NotNil(t, err)
	info, ok = ExtractInterruptInfo(err)
	assert.True(t, ok)
	assert.Equal(t, &InterruptInfo{
...
func TestNestedSubGraph(t *testing.T) {
	...
	_, err = r.Invoke(ctx, "start", WithCheckPointID("1", nil))
	assert.NotNil(t, err)
	info, ok := ExtractInterruptInfo(err)
	assert.True(t, ok)
	assert.Equal(t, &InterruptInfo{
	...
	_, err = r.Invoke(ctx, "start", WithCheckPointID("1", func(ctx context.Context, path []string, state any) error {
		if times == 0 {
			...
			assert.Equal(t, []string{"2", "2"}, path)
			state.(*testStruct).A = "state"
		}
		times++
		return nil
