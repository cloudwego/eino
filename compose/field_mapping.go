...
// allowedFields defines the whitelist of fields that can be accessed via reflection
var allowedFields = map[string]bool{
    // Add all legitimate field names here
    "Name": true,
    "Type": true,
    "Value": true,
...
// isAllowedFieldName validates if a field name is in the whitelist
func isAllowedFieldName(fieldName string) bool {
...
// safeFieldByName safely accesses a field by name with validation
func safeFieldByName(v reflect.Value, fieldName string) reflect.Value {
	if v.Kind() != reflect.Struct {
		return reflect.Value{}
	}
	
	// Check if field exists and is exported
	field, ok := v.Type().FieldByName(fieldName)
	if !ok || field.PkgPath != "" { // PkgPath is empty for exported fields
