...
type nodeKeyKey struct{}
type stateModifierKey struct{}
type checkPointKey struct{} // *checkpoint

func getNodeKey(ctx context.Context) ([]string, bool) {
	if key, ok := ctx.Value(nodeKeyKey{}).([]string); ok {
		return key, true
	}
	return nil, false
}
...
func setNodeKey(ctx context.Context, key string) context.Context {
	keys, existed := getNodeKey(ctx)
	if !existed || len(keys) == 0 {
		return context.WithValue(ctx, nodeKeyKey{}, []string{key})
	}
	return context.WithValue(ctx, nodeKeyKey{}, append(keys, key))
}
