...
1. Remove the `State` alias and `State*` constants if the capability remains
   unreleased. If compatibility is already required, add `Deprecated:` doc
   comments to every alias.
2. Replace `ToolStreamEvent.Type string` with a typed discriminator and exported
   constants. The current wire contract relies on undocumented string literals.
3. Document limit normalization for `ReadRecentTaskEvents`, `ListPending`,
   inbox listing, and outbox receive APIs. Current concrete defaults are not
   part of the interface contract.
4. Rename `NotificationDeliveryRuntime` to
...
5. Mark legacy filesystem `Config` itself deprecated, not only
...
| `StatusPending`, `StatusRunning`, `StatusWaitingInput`, `StatusSuspended`, `StatusCompleted`, `StatusFailed`, `StatusCanceled` | Typed constants | **Keep** | Complete closed set with meaningful names. Unknown values are rejected at Store boundaries. |
| `State` | Alias of `Status` | **Compatibility** | Duplicates the canonical name. Remove while unreleased or mark deprecated. |
| `StatePending`, `StateRunning`, `StateWaitingInput`, `StateSuspended`, `StateCompleted`, `StateFailed`, `StateCanceled` | Aliases of `Status*` | **Compatibility** | Seven extra names with no semantic value. They also lack individual deprecation comments. |
...
| `ControlRequest` | Fields: `Kind`, `Reason` | **Keep; document** | `Reason` is meaningful only for timeout today; document ignored/required behavior by kind. |
...
|---|---|---|
| `ErrNotFound` | **Keep** | Stable sentinel needed by Store and Manager callers. |
...
| `Task` | Fields: `Spec`, `LeaseExpiryPolicy`, `Status`, `Checkpoint`, `ResultData`, `ResultError`, `OutputFileErr`, `PendingResume`, `Version`, `Attempt`, `CancelRequestedAt`, `UpdatedAt`, `DoneAt` | **Keep; document** | Necessary snapshot. Field comments are generally strong; add explicit immutability/copy ownership for byte slices and time pointers returned by Stores. |
| `Clock` | `func() time.Time` | **Keep** | Small, useful Store testability seam. |
...
| `CancelTaskRequest` | `TaskID`, `ExpectedVersion` | **Keep** | Correct active-attempt cancellation acknowledgement. |
| `RequestCancelRequest` | `TaskID`, `ExpectedVersion` | **Keep** | Correct durable intent transition, distinct from acknowledgement. |
| `ResumeRequest` | `TaskID`, `ExpectedVersion`, `Data` | **Keep** | Correct one-shot external input command. |
| `ReleaseSuspensionRequest` | `TaskID`, `ExpectedVersion` | **Keep** | Minimal planned-resume transition. |
| `WaitUpdateRequest` | `TaskID`, `AfterVersion` | **Keep; document** | State explicitly that only lifecycle version changes wake this wait; task events do not. |
...
| `CreateAndStart` | **Keep** | Needed to avoid a process-local closure becoming durable but unresolvable between create and registration/start. |
...
| `Complete`, `Fail`, `WaitInput`, `Suspend`, `Yield`, `Cancel` | **Keep** | Explicit semantic transitions are preferable to a generic update API. |
...
| `ReleaseSuspension` | **Keep** | Explicit planned redispatch operation. |
| `Wait` | **Keep; document** | Specify spurious wake behavior, context errors, and lifecycle-only semantics. |
...
| `Manager.WaitUpdate` | **Keep; document** | Explicitly document that progress events do not advance lifecycle version. |
...
| Exported API | Shape | Verdict | Audit |
|---|---|---|---|
| `InMemoryStoreConfig` | Fields: `Clock`, `ActiveAttemptTimeout`, `MaxValueBytes` | **Keep; document** | Useful deterministic provider configuration. Document defaults and which values count toward `MaxValueBytes`. |
...
| `NotificationTarget` | Fields: `Kind`, `TargetID`, `Metadata` | **Keep; document** | Good extensible route. Document namespaced metadata and size bounds on the type, not only in validation code. |
| `Notification` | Fields: `ID`, `TaskID`, `Version`, `Kind`, `Target`, `CreatedAt`, `Task` | **Keep; document** | Correct pointer plus optional enriched snapshot. `Task`'s nil/populated phases need stronger method-level guarantees. |
| `ReceiveNotificationsRequest` | Fields: `ConsumerID`, `Limit`, `VisibilityTime` | **Keep; document** | Standard lease API; define defaults, maximums, and redelivery timing. |
| `NotificationReceipt` | Opaque `[]byte` | **Keep** | Correct provider-owned acknowledgement token. Document caller copy/retention rules. |
...
| `NotificationOutbox` | Methods: `Receive`, `Ack` | **Keep; document** | Clean SPI; needs reusable conformance tests for visibility and acknowledgement. |
| `NotificationSink` | Method: `Accept` | **Keep** | Minimal delivery target. |
| `RoutedNotificationSink` | Method: `AcceptTarget` | **Rework** | Parallel optional interface works, but `Accept` becomes a deliberately rejecting method for routed sinks. Prefer one sink method that always receives the target. |
| `ValidatingNotificationSink` | Embeds `NotificationSink`; method `ValidateNotificationSink` | **Keep** | Reasonable optional structural validation capability. |
| `NotificationSinkRegistry` | Method: `Resolve` | **Keep** | Minimal routing lookup. |
| `SinkRegistry` | In-memory registry | **Keep** | Useful default implementation. |
| `NewSinkRegistry`, `SinkRegistry.Register`, `SinkRegistry.Resolve` | Constructor and methods | **Keep** | Conventional, minimal registry API. |
| `Dispatcher` | Fields: `Outbox`, `Store`, `Sinks`, `ConsumerID`, `BatchSize`, `Visibility` | **Keep; document** | Useful host component. Public mutable fields make post-construction invalid states possible; a validated constructor would be safer. |
| `Dispatcher.DispatchOnce` | One-batch dispatch | **Keep** | Good composable primitive. |
| `NotificationDeliveryValidation` | Fields: `OutboxAvailable`, `TargetKind` | **Rework** | Shape is adequate, but it exposes internal proof mechanics. Prefer a direct `ValidateRoute(ctx, targetKind)` contract. |
| `NotificationDeliveryRuntime` | Method: `ValidateNotificationDelivery` | **Rework** | It is a validator, not a delivery runtime. Rename and simplify with the validation request. |
| `SessionInboxNotificationKind` | String constant | **Keep** | Required stable route discriminator. |
...
| `Runner` | Opaque process-local closure registry | **Keep** | Correctly owns non-serializable work outside the Store. |
...
## 3. Session Notification Adapter: `adk/backgroundtask/sessionnotify`
...
| `Sink` | Durable enqueue then activation adapter | **Keep** | Cohesive two-step delivery component. |
| `NewSink` | Validated constructor | **Keep** | Prevents incomplete sink wiring. |
| `Sink.Accept` | Rejects unrouted delivery | **Rework** | Public method exists only to satisfy `NotificationSink`, then rejects. This demonstrates tension in the two-interface sink design. |
| `Sink.AcceptTarget` | Routed delivery | **Keep** | This is the real sink operation and has the right inputs. |
| `Sink.ValidateNotificationSink` | Structural validation | **Keep** | Appropriate optional capability. |
| `Runtime` | Construction-time route proof | **Rework** | Name is too broad; use `DeliveryValidator` or `RouteValidator`. |
| `NewRuntime` | Fields as arguments: sink registry and readiness callback | **Rework** | The callback is opaque and the resulting type only validates. A named config would improve extensibility if more checks are expected. |
| `Runtime.ValidateNotificationDelivery` | Validation method | **Rework** | Rename with the interface and simplify the request shape. |
...
| `ProgressReader.ReadProgress` | Bounded textual projection | **Keep** | Correctly keeps model presentation outside the Store. |
...
| `Worker.Run` | Poll and dispatch loop | **Keep** | Concurrency and shutdown semantics are good; Store authorization remains authoritative. |
...
| `TypedConfig[M]` | Fields: `Manager`, `Notifications`, `ReadTaskProgress`, `ProgressReaders`, `TaskOutputToolConfig`, `TaskStopToolConfig` | **Rework mildly** | Overall coherent. `ReadTaskProgress` plus `ProgressReaders` creates two selection mechanisms; deprecate the fallback once all domains register by executor key. |
...
| Exported API | Shape | Verdict | Audit |
|---|---|---|---|
| `RunnerSessionID` | `func(context.Context) (string, bool)` | **Keep** | Narrow request-scoped identity lookup for model-facing notification routing. It does not expose stores or configuration through context. |
...
| `NewDurableTaskProgressHook` | Accepts `SessionEventStore` and formatter; returns progress callback | **Rework mildly** | Store injection is explicit. Returning a bare function still prevents capability discovery/config evolution; a `TaskProgressReader` implementation would align with control middleware. |
| `NewTyped` | Typed middleware constructor | **Keep** | Appropriate. |
...
| `task_stop` | Input: `task_id` | **Keep** | Minimal cancellation request. |
...
|---|---|---|
| `Manager`, `Store`, `Executor`, `Worker`, `Dispatcher` | **Keep** | Roles are conventional and distinct. |
| `Status` versus `State` | **Remove alias** | Two names for one lifecycle concept are unnecessary. |
| `TaskEvent` | **Keep** | Correctly signals task progress without implying lifecycle authority. |
| `Output*` names (`OutputFile`, `OutputFileErr`, `OutputMaterializer`, `ReportOutputFailure`) | **Keep; clarify** | These mean derived transcript/file output, while `TaskEvent` means progress. Package docs should state this distinction once. |
| `NotificationDeliveryRuntime` | **Rename** | It validates a route; it is not the dispatcher/runtime. |
...
- `Store` documents cancellation, fencing, yield, and EventID replay semantics.
...
- DeepAgent and sub-agent middleware document top-level ownership and
...
- `ExecutionRuntime.AppendTaskEvent` does not document empty-ID generation.
...
- Compatibility aliases do not use standard `Deprecated:` comments.
...
| Minimum API surface | 3/5 | `ValidateResume`, `State*`, compatibility aliases, and duplicated configs add avoidable surface. |
...
| Naming | 3/5 | Mostly precise; `State`, `Runtime`, local `Type`, and DeepAgent `Durable` need correction. |
