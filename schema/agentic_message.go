...
type AgenticMessage struct {
	MessageMeta *AgenticResponseMeta
...
type AssistantGenText struct {
	...
	Annotation AssistantGenTextAnnotation

	// Extra stores additional information.
	Extra map[string]any
}

type AssistantGenTextAnnotation interface {
	ImplAssistantGenTextAnnotation()
}
...
type ServerToolCall struct {
	...
	// supplied by the component's implementer.
	Arguments BaseServerToolCall

	// Extra stores additional information.
	Extra map[string]any
}

type BaseServerToolCall interface {
	ImplBaseServerToolCall()
}
...
type ServerToolResult struct {
	...
	// supplied by the component's implementer.
	Result BaseServerToolResult

	// Extra stores additional information.
	Extra map[string]any
}

type BaseServerToolResult interface {
	ImplBaseServerToolResult()
}
...
func ConcatAgenticMessages(messages []*AgenticMessage) (ret *AgenticMessage, err error) {
	...
	var (
		genTextBlocks        []*AssistantGenText
		genTextBlocksIndex   []int
		genTextBlocksInIndex = map[int][]*AssistantGenText{}
	)
	...
	for _, msg := range messages {
		for _, block := range msg.Blocks {
			...
			case ContentBlockTypeAssistantGenText:
				...
				if idx := block.StreamMeta.Index; idx != nil {
	...
	for _, idx := range genTextBlocksIndex {
		genText, err := concatAssistantGenText(genTextBlocksInIndex[idx])
		if err != nil {
			return nil, err
		}
		if genText == nil {
	...
	if len(genTextBlocks) > 0 {
		genText, err := concatAssistantGenText(genTextBlocks)
		if err != nil {
			return nil, err
		}
		ret.Blocks = append(ret.Blocks, &ContentBlock{
...
func concatAssistantGenText(blocks []*AssistantGenText) (final *AssistantGenText, err error) {
	if len(blocks) == 0 {
		return nil, nil
	}

	final = &AssistantGenText{}

	annotations := make([]AssistantGenTextAnnotation, 0, len(blocks))
	...
	final.Annotation, err = internal.ConcatItems(annotations)
	if err != nil {
		return nil, err
	}
