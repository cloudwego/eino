...
type MCPListToolsItem struct {
	...
	// InputSchema is the
	//JSON schema that describes the tool input.
...
func NewContentBlock[T contentBlockVariant](block *T) *ContentBlock {
...
func concatAgenticResponseMeta(metas []*AgenticResponseMeta) (ret *AgenticResponseMeta, err error) {
	...
	for _, meta := range metas {
		...
		if meta.Extension != nil {
			extType_ := reflect.TypeOf(meta.Extension)
			if extType == nil {
				extType = extType_
				extensions = reflect.MakeSlice(reflect.SliceOf(extType), 0, len(metas))
			} else if extType != extType_ {
				return nil, fmt.Errorf("expected extension type '%s', but got '%s'", extType, extType_)
			}
			extensions = reflect.Append(extensions, reflect.ValueOf(meta.Extension))
		}

		if meta.OpenAIExtension != nil {
			extType_ := reflect.TypeOf(meta.OpenAIExtension)
			if extType == nil {
				extType = extType_
			} else if extType != extType_ {
				return nil, fmt.Errorf("expected extension type '%s', but got '%s'", extType, extType_)
		...
		if meta.ClaudeExtension != nil {
			extType_ := reflect.TypeOf(meta.ClaudeExtension)
			if extType == nil {
				extType = extType_
			} else if extType != extType_ {
				return nil, fmt.Errorf("expected extension type '%s', but got '%s'", extType, extType_)
		...
		if meta.GeminiExtension != nil {
			extType_ := reflect.TypeOf(meta.GeminiExtension)
			if extType == nil {
				extType = extType_
			} else if extType != extType_ {
				return nil, fmt.Errorf("expected extension type '%s', but got '%s'", extType, extType_)
...
func concatTokenUsage(usages []*TokenUsage) *TokenUsage {
	...
	for _, usage := range usages {
		...
		ret.CompletionTokenDetails.ReasoningTokens += usage.CompletionTokenDetails.ReasoningTokens
...
func concatContentBlockHelper[T contentBlockVariant](
	...
	items, err := genericGetTFromContentBlocks(blocks, func(block *ContentBlock) (*T, error) {
		if block.Type != expectedType {
			return nil, fmt.Errorf("expected '%s' block, but got '%s'", expectedType, block.Type)
		}
		item := getter(block)
		if item == nil {
...
func concatAssistantGenTexts(texts []*AssistantGenText) (ret *AssistantGenText, err error) {
	...
	for _, t := range texts {
		...
		if t.Extension != nil {
			extType_ := reflect.TypeOf(t.Extension)
			if extType == nil {
				extType = extType_
				extensions = reflect.MakeSlice(reflect.SliceOf(extType), 0, len(texts))
			} else if extType != extType_ {
				return nil, fmt.Errorf("expected extension type '%s' for assistant generated text, but got '%s'", extType, extType_)
			}
			extensions = reflect.Append(extensions, reflect.ValueOf(t.Extension))
		}

		if t.OpenAIExtension != nil {
			extType_ := reflect.TypeOf(t.OpenAIExtension)
			if extType == nil {
				extType = extType_
			} else if extType != extType_ {
				return nil, fmt.Errorf("expected extension type '%s' for assistant generated text, but got '%s'", extType, extType_)
		...
		if t.ClaudeExtension != nil {
			extType_ := reflect.TypeOf(t.ClaudeExtension)
			if extType == nil {
				extType = extType_
			} else if extType != extType_ {
				return nil, fmt.Errorf("expected extension type '%s' for assistant generated text, but got '%s'", extType, extType_)
...
func concatAssistantGenImages(images []*AssistantGenImage) (*AssistantGenImage, error) {
	...
	for _, img := range images {
		...
		if ret.URL == "" {
			...
			return nil, fmt.Errorf("expected URL '%s' for assistant generated image, but got '%s'", ret.URL, img.URL)
		}

		if ret.MIMEType == "" {
			ret.MIMEType = img.MIMEType
		} else if img.MIMEType != "" && ret.MIMEType != img.MIMEType {
			return nil, fmt.Errorf("expected MIME type '%s' for assistant generated image, but got '%s'", ret.MIMEType, img.MIMEType)
		}
	}
...
func concatAssistantGenAudios(audios []*AssistantGenAudio) (*AssistantGenAudio, error) {
	...
	for _, audio := range audios {
		...
		if ret.URL == "" {
			...
			return nil, fmt.Errorf("expected URL '%s' for assistant generated audio, but got '%s'", ret.URL, audio.URL)
		}

		if ret.MIMEType == "" {
			ret.MIMEType = audio.MIMEType
		} else if audio.MIMEType != "" && ret.MIMEType != audio.MIMEType {
			return nil, fmt.Errorf("expected MIME type '%s' for assistant generated audio, but got '%s'", ret.MIMEType, audio.MIMEType)
		}
	}
...
func concatAssistantGenVideos(videos []*AssistantGenVideo) (*AssistantGenVideo, error) {
	...
	for _, video := range videos {
		...
		if ret.URL == "" {
			...
			return nil, fmt.Errorf("expected URL '%s' for assistant generated video, but got '%s'", ret.URL, video.URL)
		}

		if ret.MIMEType == "" {
			ret.MIMEType = video.MIMEType
		} else if video.MIMEType != "" && ret.MIMEType != video.MIMEType {
			return nil, fmt.Errorf("expected MIME type '%s' for assistant generated video, but got '%s'", ret.MIMEType, video.MIMEType)
		}
	}
...
func concatMCPToolCalls(calls []*MCPToolCall) (*MCPToolCall, error) {
	...
	for _, c := range calls {
		...
		} else if c.ApprovalRequestID != "" && c.ApprovalRequestID != ret.ApprovalRequestID {
			return nil, fmt.Errorf("expected approval request ID '%s' for mcp tool call, but got '%s'", ret.ApprovalRequestID, c.ApprovalRequestID)
		}
