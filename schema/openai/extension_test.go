...
func TestConcatResponseMetaExtensions(t *testing.T) {
	t.Run("multiple extensions - takes last non-empty values", func(t *testing.T) {
		...
		streamErr := &StreamingResponseError{Code: "stream_err", Message: "stream msg"}
		...
		assert.Equal(t, streamErr, result.StreamingError)
		assert.Equal(t, incomplete, result.IncompleteDetails)
	})

	t.Run("streaming scenario", func(t *testing.T) {
		exts := []*ResponseMetaExtension{
			{ID: "chatcmpl_stream", Status: "", Error: nil, StreamingError: nil, IncompleteDetails: nil},
			{ID: "", Status: ResponseStatus("in_progress"), Error: nil, StreamingError: nil, IncompleteDetails: nil},
			{ID: "", Status: ResponseStatus("completed"), Error: nil, StreamingError: nil, IncompleteDetails: nil},
		}
