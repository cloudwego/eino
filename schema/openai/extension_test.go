...
func TestConcatResponseMetaExtensions(t *testing.T) {
	t.Run("multiple extensions - takes last non-empty values", func(t *testing.T) {
		...
		streamErr := &StreamResponseError{Code: "stream_err", Message: "stream msg"}
		...
		assert.Equal(t, streamErr, result.StreamError)
		assert.Equal(t, incomplete, result.IncompleteDetails)
	})

	t.Run("streaming scenario", func(t *testing.T) {
		exts := []*ResponseMetaExtension{
			{ID: "chatcmpl_stream", Status: "", Error: nil, StreamError: nil, IncompleteDetails: nil},
			{ID: "", Status: "in_progress", Error: nil, StreamError: nil, IncompleteDetails: nil},
			{ID: "", Status: "completed", Error: nil, StreamError: nil, IncompleteDetails: nil},
		}
