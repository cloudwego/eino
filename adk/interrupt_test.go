...
func TestSimpleInterrupt(t *testing.T) {
	...
	agent := &myAgent{
		...
		resumer: func(ctx context.Context, info *InterruptInfo, opts ...AgentRunOption) *AsyncIterator[*AgentEvent] {
...
func TestMultiAgentInterrupt(t *testing.T) {
	...
	sa2 := &myAgent{
		...
		resumer: func(ctx context.Context, info *InterruptInfo, opts ...AgentRunOption) *AsyncIterator[*AgentEvent] {
			...
			generator.Send(&AgentEvent{
				AgentName: "sa2",
				Output: &AgentOutput{
					ModelResponse: &MessageVariant{Message: schema.UserMessage("completed")},
				},
			})
			generator.Close()
			return iter
	...
	event, ok = iter.Next()
	assert.True(t, ok)
	assert.Equal(t, event.Output.ModelResponse.Message.Content, "completed")
	_, ok = iter.Next()
	assert.False(t, ok)
}
...
func TestChatModelInterrupt(t *testing.T) {
	assert.NoError(t, compose.RegisterSerializableType[State]("_eino_adk_state"))
	assert.NoError(t, compose.RegisterSerializableType[AgentAction]("_eino_adk_agent_action"))
	assert.NoError(t, compose.RegisterSerializableType[AgentEvent]("_eino_adk_agent_event"))
	assert.NoError(t, compose.RegisterSerializableType[agentToolInterruptInfo]("_eino_adk_agent_tool_interrupt_info"))
	ctx := context.Background()
	...
	assert.True(t, ok)
	assert.NoError(t, event.Err)
	assert.Equal(t, event.Output.ToolCallResponse.Response.Message.Content, "result")
	event, ok = iter.Next()
	assert.True(t, ok)
	assert.NoError(t, event.Err)
	assert.Equal(t, event.Output.ModelResponse.Message.Content, "completed")
...
func TestChatModelAgentToolInterrupt(t *testing.T) {
	...
	sa := &myAgent{
		...
		resumer: func(ctx context.Context, info *InterruptInfo, opts ...AgentRunOption) *AsyncIterator[*AgentEvent] {
			...
			iter, generator := NewAsyncIteratorPair[*AgentEvent]()
			generator.Send(&AgentEvent{Output: &AgentOutput{ModelResponse: &MessageVariant{Message: schema.UserMessage("my agent completed")}}})
			generator.Close()
			return iter
		},
	}
	...
	assert.True(t, ok)
	assert.NoError(t, event.Err)
	assert.Equal(t, event.Output.ToolCallResponse.Response.Message.Content, "my agent completed")
	event, ok = iter.Next()
	assert.True(t, ok)
	assert.NoError(t, event.Err)
	assert.Equal(t, event.Output.ModelResponse.Message.Content, "completed")
...
func (m *myAgent) Resume(ctx context.Context, info *InterruptInfo, opts ...AgentRunOption) *AsyncIterator[*AgentEvent] {
