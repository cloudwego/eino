...
func (r *Runner) handleIter(ctx context.Context, aIter *AsyncIterator[*AgentEvent],
	...
	for {
		...
		// CancelError handling: cancel terminates the event stream
		if event.Err != nil {
			...
			if errors.As(event.Err, &cancelErr) {
				...
				interruptSignal = cancelErr.interruptSignal

				if checkPointID != nil && interruptSignal != nil {
				...
				// Forward CancelError as-is to the consumer
				gen.Send(event)
