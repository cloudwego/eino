...
func (r *Runner) Run(ctx context.Context, messages []Message,
	...
	go r.run(ctx, iter, gen, o.checkPointID)
	return niter
}
...
func (r *Runner) Resume(ctx context.Context, checkPointID string, opts ...AgentRunOption) (*AsyncIterator[*AgentEvent], error) {
	...
	go r.run(ctx, aIter, gen, &checkPointID)
	return niter, nil
}

func (r *Runner) run(ctx context.Context, aIter *AsyncIterator[*AgentEvent], gen *AsyncGenerator[*AgentEvent], checkPointID *string) {
	defer func() {
		panicErr := recover()
		if panicErr != nil {
