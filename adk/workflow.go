...
func (a *workflowAgent) Run(ctx context.Context, input *AgentInput, _ ...Option) *AsyncIterator[*AgentEvent] {
...
func newWorkflowAgent(ctx context.Context, config *WorkflowAgentConfig,
	mode workflowAgentMode, maxIterations int) (*flowAgent, error) {
	...
	fas := make([]Agent, len(config.SubAgents))
	for i, subAgent := range config.SubAgents {
...
func NewSequentialAgent(ctx context.Context, config *SequentialAgentConfig) (Agent, error) {
	return newWorkflowAgent(ctx, &config.WorkflowAgentConfig, workflowAgentModeSequential, 0)
}

func NewParallelAgent(ctx context.Context, config *ParallelAgentConfig) (Agent, error) {
	return newWorkflowAgent(ctx, &config.WorkflowAgentConfig, workflowAgentModeParallel, 0)
}

func NewLoopAgent(ctx context.Context, config *LoopAgentConfig) (Agent, error) {
	return newWorkflowAgent(ctx, &config.WorkflowAgentConfig, workflowAgentModeLoop, config.MaxIterations)
}
