...
func (a *workflowAgent) runSequential(ctx context.Context, input *AgentInput,
	...
	for ; i < len(a.subAgents); i++ {
		...
		if lastActionEvent != nil {
			...
			if breakLoopIfNeeded(lastActionEvent.Action) {
...
type BreakLoopAction struct {
	// By records the name of the agent that initiated the break loop action.
	By string
...
func breakLoopIfNeeded(a *AgentAction) bool {
	if a != nil && a.BreakLoop != nil && !a.BreakLoop.Done {
		a.BreakLoop.Done = true
		return true
	}
	return false
}
