...
func TestCallbackOnStartInvocation(t *testing.T) {
	...
	chatAgent, err := NewChatModelAgent(ctx, &ChatModelAgentConfig{
		Name:        "TestAgent",
	...
	agent := NewMultiAgent(ctx, "TestAgent", "Test agent for callback", chatAgent)

	recorder := &callbackRecorder{}
	handler := newRecordingHandler(recorder)
	input := &AgentInput{Messages: []Message{schema.UserMessage("hello")}}

	iter := agent.Run(ctx, input, WithCallbacks(handler))
	for {
		_, ok := iter.Next()
		if !ok {
			break
...
func TestCallbackRunInfoForChatModelAgent(t *testing.T) {
	...
	agent := NewMultiAgent(ctx, "TestChatAgent", "Test chat agent", chatAgent)
...
func TestMultipleCallbackHandlers(t *testing.T) {
	...
	agent := NewMultiAgent(ctx, "TestAgent", "Test agent", chatAgent)
	...
	iter := agent.Run(ctx, input, WithCallbacks(handler1, handler2))
	for {
		_, ok := iter.Next()
		if !ok {
			break
...
func TestCallbackWithWorkflowAgent(t *testing.T) {
	...
	iter := seqAgent.Run(ctx, input, WithCallbacks(handler))
	for {
		_, ok := iter.Next()
		if !ok {
			break
...
func TestCallbackWithMultiAgent(t *testing.T) {
	ctx := context.Background()

	events := []*AgentEvent{
		{Output: &AgentOutput{MessageOutput: &MessageVariant{Message: schema.AssistantMessage("hello", nil)}}},
	}
	innerAgent := &simpleTestAgent{name: "inner", description: "inner agent", events: events}
	ma := NewMultiAgent(ctx, "MultiTestAgent", "Multi agent test", innerAgent)

	recorder := &callbackRecorder{}
	handler := newRecordingHandler(recorder)
	input := &AgentInput{Messages: []Message{schema.UserMessage("hello")}}

	iter := ma.Run(ctx, input, WithCallbacks(handler))
	for {
		_, ok := iter.Next()
		if !ok {
			break
	...
	assert.True(t, recorder.onStartCalled, "OnStart should be called")
	assert.NotNil(t, recorder.runInfo)
	assert.Equal(t, "inner", recorder.runInfo.Name)
}
