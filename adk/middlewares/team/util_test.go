...
func TestMultiError_ErrorsIs(t *testing.T) {
	// Unwrap() []error is only recognized by errors.Is since Go 1.20.
	// Under Go 1.19 we can only verify that Error() contains sub-error messages.
	sentinel := errors.New("sentinel")
	other := errors.New("other")
	combined := joinErrors(sentinel, other)
	assert.Contains(t, combined.Error(), "sentinel")
	assert.Contains(t, combined.Error(), "other")
}
