...
func (s *ChanSource) Receive(ctx context.Context,
	...
	select {
	case <-ctx.Done():
		log.Printf("Receive: ctx.Err() = %v", ctx.Err())
	...
	item, ok := <-s.ch
	if !ok {
		log.Printf("Receive: not enough messages in channel")
		return ctx, TurnInput{}, nil, fmt.Errorf("not enough messages in channel")
	}
