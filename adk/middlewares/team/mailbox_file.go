...
// Injected at construction so the mailbox doesn't depend on teamConfigStore directly.
...
func newMailboxFromConfig(conf *Config, teamName, ownerName string) *mailbox {
	...
	return &mailbox{
		...
		listMembers: func(ctx context.Context) ([]string, error) {
			var names []string
			err := cfgStore.readConfigWithReadLock(ctx, teamName, func(cfg *teamConfig) error {
...
// Messages are matched by from+to+text+summary+timestamp.
