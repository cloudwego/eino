...
func TestParseMessageType_ValidTypes(t *testing.T) {
	tests := []struct {
		...
		{"shutdown_approved", messageTypeShutdownApproved},
		{"plan_approval_response", messageTypePlanApprovalResponse},
	}
...
func TestMarshalShutdownApproval_Approve(t *testing.T) {
	s, err := marshalShutdownApproval("leader", "req-2", true, "approved reason")
	assert.NoError(t, err)

	var m map[string]interface{}
	assert.NoError(t, json.Unmarshal([]byte(s), &m))
	assert.Equal(t, "shutdown_approved", m["type"])
	assert.Equal(t, "leader", m["from"])
...
func TestMarshalShutdownApproval_Reject(t *testing.T) {
	s, err := marshalShutdownApproval("leader", "req-3", false, "not yet")
	assert.NoError(t, err)
...
func TestMarshalPlanApprovalResponse_Approve(t *testing.T) {
	s, err := marshalPlanApprovalResponse("agent-a", "req-10", true, "looks good")
	assert.NoError(t, err)

	var m map[string]interface{}
	assert.NoError(t, json.Unmarshal([]byte(s), &m))
	assert.Equal(t, "plan_approval_response", m["type"])
	assert.Equal(t, "agent-a", m["from"])
	assert.Equal(t, "req-10", m["requestId"])
	assert.Equal(t, true, m["approve"])
	assert.Equal(t, "looks good", m["feedback"])
}

func TestMarshalPlanApprovalResponse_Reject(t *testing.T) {
	s, err := marshalPlanApprovalResponse("agent-b", "req-11", false, "needs changes")
	assert.NoError(t, err)

	var m map[string]interface{}
	assert.NoError(t, json.Unmarshal([]byte(s), &m))
	assert.Equal(t, false, m["approve"])
	assert.Equal(t, "needs changes", m["feedback"])
}

func TestDecodeShutdownApproval_Valid(t *testing.T) {
	input := `{"type":"shutdown_approved","from":"leader","requestId":"r1","timestamp":"2025-01-01T00:00:00.000Z","approve":true,"reason":"ok"}`
	p, err := decodeShutdownApproval(input)
	assert.NoError(t, err)
	assert.Equal(t, "shutdown_approved", p.Type)
...
func TestDecodeShutdownApproval_InvalidJSON(t *testing.T) {
	_, err := decodeShutdownApproval("not json")
	assert.Error(t, err)
}
...
func TestSendMessageTypeRules_ShutdownApproved(t *testing.T) {
	rule := sendMessageTypeRules[messageTypeShutdownApproved]
	assert.True(t, rule.requiresRecipient)
	assert.False(t, rule.requiresContent)
	assert.False(t, rule.requiresSummary)
...
func TestSendMessageTypeRules_PlanApprovalResponse(t *testing.T) {
	rule := sendMessageTypeRules[messageTypePlanApprovalResponse]
	assert.True(t, rule.requiresRecipient)
	assert.False(t, rule.requiresContent)
	assert.False(t, rule.requiresSummary)
	assert.True(t, rule.requiresRequestID)
