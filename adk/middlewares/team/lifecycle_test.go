...
func TestLifecycleManager_ConfigStore(t *testing.T) {
	...
	assert.Same(t, conf.state.cfgStore, lm.configStore())
}
...
func TestLifecycleManager_StopTeammateRuntime(t *testing.T) {
	...
	lm.registry.register("worker", &teammateHandle{Cancel: cancel, LocalTaskID: ""})
...
func TestLifecycleManager_RemoveTeammate(t *testing.T) {
	...
	unassigned, err := lm.removeTeammate(ctx, teamName, "worker")
	assert.NoError(t, err)
...
func TestLifecycleManager_RemoveTeammate_UnassignError(t *testing.T) {
	...
	_, err = lm.removeTeammate(ctx, teamName, "worker")
	assert.Error(t, err)
