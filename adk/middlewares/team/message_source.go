...
func (s *MailboxMessageSource) handleLeaderControlMessages(ctx context.Context, msgs []InboxMessage) ([]InboxMessage, error) {
	...
	for _, m := range msgs {
		...
		// notifyMsg == "" means the teammate was already cleaned up by
		// cleanupExitedTeammate (which sent its own notification via the
		// router). Skip creating a duplicate system message.
		if notifyMsg == "" {
			continue
		}
