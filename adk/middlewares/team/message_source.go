...
// (shutdown approval, teammate terminated), and builds TurnInput items.
...
func (s *MailboxMessageSource) tryReceive(ctx context.Context, notifyIdle bool) (TurnInput, bool, error) {
	...
	if len(msgs) == 0 {
		if notifyIdle && s.conf.Role == teamRoleTeammate && s.processedCount > s.lastIdleProcessedCount {
			...
			_ = sendIdleNotification(ctx, s.mailbox, s.conf.OwnerName, "available")
		}
		return TurnInput{}, false, nil
	}
...
func (s *MailboxMessageSource) waitForItem(ctx context.Context) (TurnInput, error) {
	...
	for {
		// Check on every iteration so that the leader exits promptly when the
		// last teammate shuts down, even if no new inbox messages arrive.
		if s.conf.Role == teamRoleLeader && s.conf.ExitWhenNoTeammates && s.conf.HasActiveTeammates != nil {
			...
			if !active {
				return empty, fmt.Errorf("no active teammates")
			}
		}
...
func (s *MailboxMessageSource) consumeMessages(ctx context.Context, msgs []InboxMessage) (TurnInput, bool, error) {
	...
	// Mark all original messages as read regardless of whether any remain after
	// control-message filtering. This prevents re-processing already-handled messages.
