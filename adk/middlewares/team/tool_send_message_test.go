...
func TestSendMessageTool_InvokableRun_ShutdownApproved_MissingFields(t *testing.T) {
	mw, _ := newTestTeamMiddleware()
	ctx := context.Background()
	...
	_, err = tool.InvokableRun(ctx, `{"type":"shutdown_approved","recipient":"worker"}`)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "'request_id' is required")

	_, err = tool.InvokableRun(ctx, `{"type":"shutdown_approved","recipient":"worker","request_id":"req-1"}`)
	assert.Error(t, err)
...
func TestBuildApprovalResultMessage_ShutdownApproved(t *testing.T) {
	msg := buildApprovalResultMessage(messageTypeShutdownApproved, "worker", true)
	assert.Equal(t, "Shutdown approved", msg)
}

func TestBuildApprovalResultMessage_ShutdownRejected(t *testing.T) {
	msg := buildApprovalResultMessage(messageTypeShutdownApproved, "worker", false)
	assert.Equal(t, "Shutdown rejected", msg)
}

func TestBuildApprovalResultMessage_PlanApproved(t *testing.T) {
	msg := buildApprovalResultMessage(messageTypePlanApprovalResponse, "worker", true)
	assert.Equal(t, "Plan approved for worker", msg)
}

func TestBuildApprovalResultMessage_PlanRejected(t *testing.T) {
	msg := buildApprovalResultMessage(messageTypePlanApprovalResponse, "worker", false)
	assert.Equal(t, "Plan rejected for worker", msg)
}
...
func TestSendMessageTool_BuildOutboxMessage_ShutdownApproved(t *testing.T) {
	mw, _ := newTestTeamMiddleware()
	tool, err := newSendMessageTool(mw, "worker")
	assert.NoError(t, err)

	args := &sendMessageArgs{RequestID: "req-123", Content: "done"}
	msg, err := tool.buildOutboxMessage(messageTypeShutdownApproved, LeaderAgentName, true, args)
	assert.NoError(t, err)
	assert.Equal(t, LeaderAgentName, msg.To)
	assert.Equal(t, messageTypeShutdownApproved, msg.Type)
	assert.NotEmpty(t, msg.Text)
}

func TestSendMessageTool_BuildOutboxMessage_PlanApprovalResponse(t *testing.T) {
	mw, _ := newTestTeamMiddleware()
	tool, err := newSendMessageTool(mw, LeaderAgentName)
	assert.NoError(t, err)

	args := &sendMessageArgs{RequestID: "plan-1", Content: "looks good"}
	msg, err := tool.buildOutboxMessage(messageTypePlanApprovalResponse, "worker", true, args)
	assert.NoError(t, err)
	assert.Equal(t, "worker", msg.To)
	assert.Equal(t, messageTypePlanApprovalResponse, msg.Type)
	assert.NotEmpty(t, msg.Text)
}
...
func TestSendMessageTool_BuildResult_ShutdownApproved(t *testing.T) {
	mw, _ := newTestTeamMiddleware()
	tool, err := newSendMessageTool(mw, "worker")
	assert.NoError(t, err)

	args := &sendMessageArgs{}
	msg := &outboxMessage{To: LeaderAgentName, Type: messageTypeShutdownApproved}
	result := tool.buildResult(messageTypeShutdownApproved, LeaderAgentName, true, msg, args)
	assert.Equal(t, true, result["success"])
	assert.Equal(t, "Shutdown approved", result["message"])
}

func TestSendMessageTool_InvokableRun_PlanApprovalResponse_MissingFields(t *testing.T) {
	mw, _ := newTestTeamMiddleware()
	ctx := context.Background()
	...
	_, err = tool.InvokableRun(ctx, `{"type":"plan_approval_response","recipient":"worker"}`)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "'request_id' is required")

	_, err = tool.InvokableRun(ctx, `{"type":"plan_approval_response","recipient":"worker","request_id":"plan-1"}`)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "'approve' is required")
}
...
func TestSendMessageTool_InvokableRun_ShutdownApproved_ByLeader(t *testing.T) {
	mw, conf := newTestTeamMiddleware()
	ctx := context.Background()
	...
	result, err := tool.InvokableRun(ctx, `{"type":"shutdown_approved","recipient":"worker","request_id":"req-1","approve":true}`)
	assert.NoError(t, err)
	assert.Contains(t, result, "success")
...
func TestSendMessageTool_InvokableRun_PlanApprovalResponse_Success(t *testing.T) {
	mw, conf := newTestTeamMiddleware()
	ctx := context.Background()
	...
	result, err := tool.InvokableRun(ctx, `{"type":"plan_approval_response","recipient":"worker","request_id":"plan-1","approve":true,"content":"looks good"}`)
	assert.NoError(t, err)
	assert.Contains(t, result, "success")
	assert.Contains(t, result, "Plan approved for worker")
}
...
func TestSendMessageTool_BuildResult_PlanApprovalResponse(t *testing.T) {
	mw, _ := newTestTeamMiddleware()
	tool, err := newSendMessageTool(mw, LeaderAgentName)
	assert.NoError(t, err)

	args := &sendMessageArgs{}
	msg := &outboxMessage{To: "worker", Type: messageTypePlanApprovalResponse}
	result := tool.buildResult(messageTypePlanApprovalResponse, "worker", true, msg, args)
	assert.Equal(t, true, result["success"])
	assert.Equal(t, "Plan approved for worker", result["message"])
}
