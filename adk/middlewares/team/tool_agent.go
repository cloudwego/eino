...
func (t *agentTool) runBackground(ctx context.Context, args agentToolArgs) (string, error) {
	...
	defer func() {
		if !succeeded {
			...
			defer cleanupCancel()
			t.mw.lifecycle.cleanupFailedTeammateSpawn(cleanupCtx, teamName, args.Name, localTaskID)
		}
	}()
	...
	localTaskID = t.createShadowTask(ctx, args)

	if err := t.spawnTeammateRunner(ctx, teamName, args.Name, localTaskID, tmAgent); err != nil {
		return "", err
	}
...
// createShadowTask creates an internal shadow task for tracking the teammate. Returns the task ID or "".
func (t *agentTool) createShadowTask(ctx context.Context, args agentToolArgs) string {
	taskDesc := args.Prompt
	if runes := []rune(taskDesc); len(runes) > 100 {
		taskDesc = string(runes[:100])
	}
	taskID, createErr := t.mw.lifecycle.plantaskMW().CreateTask(ctx, &plantask.TaskInput{
		Subject:     args.Name,
	...
	t.mw.logger().Printf("createShadowTask: failed to create shadow task for %q: %v", args.Name, createErr)
	return ""
}

// spawnTeammateRunner creates the teammate's TurnLoop runner and starts it in a goroutine.
func (t *agentTool) spawnTeammateRunner(ctx context.Context, teamName, name, localTaskID string, tmAgent *adk.ChatModelAgent) error {
