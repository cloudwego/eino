...
func TestSendInitialPrompt_StoresRawPromptForSingleEnvelopeFormatting(t *testing.T) {
	...
	teamName := "myteam"
	_, err := mw.lifecycle.configStore().CreateTeam(context.Background(), teamName, "", LeaderAgentName, "")
	assert.NoError(t, err)
...
func TestAgentTool_RunBackground_FullFlow(t *testing.T) {
	...
	has, _ := conf.configStore().HasMember(context.Background(), "myteam", "worker")
	assert.True(t, has)
