...
func TestHandleLeaderControlMessages_NonLeader(t *testing.T) {
	...
	approvalJSON, _ := marshalShutdownApproval("agent1", "req-1", true, "done")
	msgs := []InboxMessage{
...
func TestHandleLeaderControlMessages_InterceptsShutdownApproved(t *testing.T) {
	backend := newInMemoryBackend()
	locks := newNamedLockManager()
	mb := &mailbox{
		conf: &mailboxConfig{
...
func TestHandleLeaderControlMessages_ShutdownApprovedFalseNotIntercepted(t *testing.T) {
	backend := newInMemoryBackend()
	locks := newNamedLockManager()
	mb := &mailbox{
		conf: &mailboxConfig{
	...
	approvalJSON, _ := marshalShutdownApproval("agent1", "req-1", false, "not done yet")
	msg := InboxMessage{From: "agent1", Text: approvalJSON, Timestamp: utcNowMillis()}
