...
// its cancel function and shadow task ID for cleanup on shutdown.
...
// It bridges the teammateRegistry with the teamConfigStore, plantask middleware,
...
type lifecycleManager struct {
	...
	cfgStore   *teamConfigStore    // team config.json operations
...
func newLifecycleManager(teamCfg *Config, runnerConf *RunnerConfig, isLeader bool, router *sourceRouter, pumpMgr *pumpManager) *lifecycleManager {
	return &lifecycleManager{
		...
		cfgStore:   teamCfg.configStore(),
		router:     router,
		pumpMgr:    pumpMgr,
		teamCfg:    teamCfg,
		runnerConf: runnerConf,
		isLeader:   isLeader,
		logger:     teamCfg.logger(),
	}
}
...
func (lm *lifecycleManager) buildTeammateAgent(ctx context.Context, agentName, teamName string) (*adk.ChatModelAgent, error) {
	...
	tmAgent, ptMW, err := buildTeamAgent(ctx, lm.runnerConf, tmMW, fmt.Sprintf("Your agent name is: %s", agentName))
	if err != nil {
...
// configStore returns the shared team config store.
func (lm *lifecycleManager) configStore() *teamConfigStore {
	return lm.cfgStore
}
...
func (lm *lifecycleManager) hasMember(ctx context.Context, teamName, memberName string) (bool, error) {
	return lm.cfgStore.HasMember(ctx, teamName, memberName)
}
...
// removes the member from config, deletes the inbox file, deletes the shadow
// task (if any), and unregisters the mailbox source and loop.
func (lm *lifecycleManager) cleanupFailedTeammateSpawn(ctx context.Context, teamName, memberName, localTaskID string) {
	if err := lm.cfgStore.RemoveMember(ctx, teamName, memberName); err != nil {
	...
	if localTaskID != "" && lm.ptMW != nil {
		_ = lm.ptMW.DeleteTask(ctx, localTaskID)
	}
	lm.pumpMgr.UnsetMailbox(memberName)
	lm.router.UnregisterLoop(memberName)
}

// stopTeammateRuntime cancels the teammate's context, deletes the shadow task,
// unregisters mailbox/loop, and releases the per-inbox lock.
// Returns true if this call was the first to stop the teammate (i.e. the
// teammateHandle was still present in the registry), false if it was already stopped
// by a prior call (idempotent).
func (lm *lifecycleManager) stopTeammateRuntime(ctx context.Context, teamName, memberName string) bool {
	...
	if firstStop {
		...
		if result.LocalTaskID != "" && lm.ptMW != nil {
			_ = lm.ptMW.DeleteTask(ctx, result.LocalTaskID)
		}
	}

	lm.pumpMgr.UnsetMailbox(memberName)
	lm.router.UnregisterLoop(memberName)
	// Release the per-inbox lock to prevent memory accumulation over many
	// teammate create/destroy cycles.
...
func (lm *lifecycleManager) cleanupExitedTeammate(ctx context.Context, teamName, memberName string) {
	...
	// notification has already been sent via OnShutdownApproval — skip to avoid
...
func (lm *lifecycleManager) removeTeammate(ctx context.Context, teamName, memberName string) (unassigned []string, firstStop bool, err error) {
	...
	unassigned, err = lm.unassignMemberTasks(ctx, memberName)
	if err != nil {
		return nil, firstStop, fmt.Errorf("unassign tasks for %q: %w", memberName, err)
	}

	if err = lm.cfgStore.RemoveMember(ctx, teamName, memberName); err != nil {
		return nil, firstStop, fmt.Errorf("remove member %q: %w", memberName, err)
	}
