...
func TestMarkRead_RemovesSpecifiedMessages(t *testing.T) {
	...
	msgs := []InboxMessage{
		{From: "leader", To: "agent1", Text: "msg1", Summary: "s1", Timestamp: "t1", Read: false},
		{From: "leader", To: "agent1", Text: "msg2", Summary: "s2", Timestamp: "t2", Read: false},
		{From: "agent2", To: "agent1", Text: "msg3", Summary: "s3", Timestamp: "t3", Read: false},
	}
	assert.NoError(t, mb.writeInbox(ctx, "agent1", msgs))
...
func TestMailboxMessageKey(t *testing.T) {
	msg := InboxMessage{
		From:      "leader",
		To:        "agent1",
		Text:      "hello",
	...
	key := mailboxMessageKey(msg)
	assert.Equal(t, "leader", key.from)
	assert.Equal(t, "agent1", key.to)
	assert.Equal(t, "hello", key.text)
	assert.Equal(t, "greet", key.summary)
	assert.Equal(t, "2025-01-01T00:00:00.000Z", key.ts)
}
...
func TestNewMailboxFromConfig(t *testing.T) {
	...
	_, err := conf.state.cfgStore.CreateTeam(ctx, teamName, "desc", LeaderAgentName, "general-purpose")
	assert.NoError(t, err)
