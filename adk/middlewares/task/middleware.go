...
// Package task provides the middleware that injects the task
...
// *background.Manager, but they must NOT inject task_output/task_stop
...
// Config configures the task control middleware for the standard
// *schema.Message message type. It is the default specialization of TypedConfig.
type Config = TypedConfig[*schema.Message]

// ProgressReader projects executor-specific progress without mutating task
// state. Implementations may be called concurrently.
type ProgressReader interface {
	ReadProgress(context.Context, *bgtask.TaskSnapshot) (string, error)
}

// TypedConfig configures the task control middleware, parameterized by
// message type.
type TypedConfig[M adk.MessageType] struct {
	// Manager is the shared task Manager whose tasks the injected
	...
	// CustomSystemPrompt fully replaces the default task instruction
...
// NewTyped creates a task control middleware parameterized by message type.
...
func NewTyped[M adk.MessageType](ctx context.Context, config *TypedConfig[M]) (adk.TypedChatModelAgentMiddleware[M], error) {
	if config == nil || config.Manager == nil {
		return nil, fmt.Errorf("task/background: Manager is required")
	}
	mgr := config.Manager
	...
	if outputEnabled {
		progressReaders := make(map[string]ProgressReader, len(config.ProgressReadersByExecutorKey))
		...
		if err != nil {
			return nil, fmt.Errorf("task/background: failed to create task_output tool: %w", err)
		}
	...
	if stopEnabled {
		...
		if err != nil {
			return nil, fmt.Errorf("task/background: failed to create task_stop tool: %w", err)
		}
...
// buildInstruction assembles the task instruction so the per-tool
...
func newTaskOutputTool(
	mgr *bgtask.Manager,
	cfg *ToolConfig,
	progressReaders map[string]ProgressReader,
...
func resolveDurableTask(
	...
	readProgress func(context.Context, *bgtask.TaskSnapshot) (string, error),
) (string, error) {
	if readProgress == nil {
		return resolveDurableTaskWithReaders(ctx, mgr, task, input, nil)
	}
	readers := map[string]ProgressReader{
		task.Spec.ExecutorKey: taskProgressReaderFunc(readProgress),
	}
	return resolveDurableTaskWithReaders(ctx, mgr, task, input, readers)
}

type taskProgressReaderFunc func(context.Context, *bgtask.TaskSnapshot) (string, error)

func (f taskProgressReaderFunc) ReadProgress(ctx context.Context, task *bgtask.TaskSnapshot) (string, error) {
	return f(ctx, task)
}
...
type taskStopInput struct {
	TaskID string `json:"task_id" jsonschema:"required" jsonschema_description:"The ID of the task to stop"`
	Reason string `json:"reason,omitempty" jsonschema_description:"Optional reason for stopping the task"`
}
...
func formatTask(task *bgtask.TaskSnapshot) string {
	result := fmt.Sprintf(
		"Task ID: %s\nDescription: %s\nStatus: %s",
