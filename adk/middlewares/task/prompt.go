...
const (
	taskOutputToolDescription = `Retrieve the output and status of a running or completed task.

- Takes a task_id parameter identifying the task
- Returns the task's output along with its status and any error
- Use this tool to check on a task or retrieve its result by task_id
`
...
`
	...
	taskStopToolDescription = `Stop a running task by its ID.

- Takes a task_id parameter identifying the task to stop
- Returns a success or failure status
- Use this tool when you need to cancel a long-running task
`
...
- A task_id means the work was created as a task. Foreground tool results
  are synchronous and do not need the output tool.`

	// %s is the registered task_output tool name.
	backgroundTaskOutputLine = "\n- Use the %s tool to check a task's status or retrieve its result by task_id."

	// %s is the registered task_stop tool name.
	backgroundTaskStopLine = "\n- Use the %s tool to cancel a running task by task_id."

	backgroundTaskPromptFooter = "\n- These tasks are running executions, not planning to-dos.\n- If a task finishes quickly, wait for its completion notification before using the output tool; do not poll immediately in a loop.\n"
