...
// The path must start with a letter, dot, slash, or tilde, followed by
...
// Backend defines the file access interface for loading Agent.md files.
...
func newLoader(backend Backend, files []string, maxBytes int) *loader {
	return &loader{
		backend:  backend,
		files:    files,
		maxBytes: maxBytes,
