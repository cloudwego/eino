...
// injectTaskReminder injects a task reminder message into the conversation history
// before the model is called, if task tools haven't been used for a while.
//
// It is invoked by typedMiddleware.BeforeModelRewriteState for *schema.Message
// agents. Task reminders are only active in shared-task (team) mode, which always
// uses *schema.Message, so this helper operates on the concrete message state.
//
...
func (m *middleware) injectTaskReminder(ctx context.Context, state *adk.ChatModelAgentState, mc *adk.ModelContext) (context.Context, *adk.ChatModelAgentState, error) {
