...
func TestTaskUpdateTool(t *testing.T) {
	...
	tool := newTaskUpdateTool(testMiddleware(backend, baseDir), &sync.Mutex{})
...
func TestTaskUpdateToolWithAssignedHook(t *testing.T) {
	ctx := context.Background()
	backend := newInMemoryBackend()
	baseDir := "/tmp/tasks"
