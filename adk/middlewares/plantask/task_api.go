...
// use Middleware.CreateTask() which shares the same lock with the tool implementations.
...
func createTaskLocked(ctx context.Context, backend Backend, baseDir string, input *TaskInput) (string, error) {
	...
	taskFileName := taskIDStr + ".json"

	for _, file := range files {
		if filepath.Base(file.Path) == taskFileName {
			return "", fmt.Errorf("CreateTask task #%d already exists", taskID)
		}
	}
	...
	if err := backend.Write(ctx, &WriteRequest{
		...
		return "", fmt.Errorf("CreateTask write task #%s failed, err: %w", taskIDStr, err)
	}
...
//
// NOTE: This function is NOT concurrency-safe on its own. For concurrent access,
// use Middleware.DeleteTask() which shares the same lock with the tool implementations.
...
func deleteTaskLocked(ctx context.Context, backend Backend, baseDir string, taskID string) error {
	...
	for _, t := range tasks {
		...
		if modified {
			...
			updatedContent, marshalErr := sonic.MarshalString(t)
			if marshalErr != nil {
				return fmt.Errorf("DeleteTask marshal task #%s failed, err: %w", t.ID, marshalErr)
			}

			taskFilePath := filepath.Join(baseDir, t.ID+".json")
			if writeErr := backend.Write(ctx, &WriteRequest{FilePath: taskFilePath, Content: updatedContent}); writeErr != nil {
				return fmt.Errorf("DeleteTask write task #%s failed, err: %w", t.ID, writeErr)
			}
		}
	}

	// Delete the task file.
	taskFilePath := filepath.Join(baseDir, taskID+".json")
	if err := backend.Delete(ctx, &DeleteRequest{FilePath: taskFilePath}); err != nil {
