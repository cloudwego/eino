...
func setupTestBackend() *InMemoryBackend {
	...
	// Create test files
	backend.Write(ctx, "/file1.txt", "line1\nline2\nline3\nline4\nline5")
	backend.Write(ctx, "/file2.go", "package main\n\nfunc main() {\n\tprintln(\"hello\")\n}")
	backend.Write(ctx, "/dir1/file3.txt", "hello world\nfoo bar\nhello again")
	backend.Write(ctx, "/dir1/file4.py", "print('hello')\nprint('world')")
	backend.Write(ctx, "/dir2/file5.go", "package test\n\nfunc test() {}")
...
func TestWriteFileTool(t *testing.T) {
	...
	ctx := context.Background()
	content, err := backend.Read(ctx, "/newfile.txt", 0, 100)
	if err != nil {
	...
	content, err = backend.Read(ctx, "/file1.txt", 0, 100)
	if err != nil {
		t.Fatalf("Failed to read written file: %v", err)
	}
...
func TestEditFileTool(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			input:        `{"file_path": "/edit1.txt", "old_string": "hello", "new_string": "hi", "replace_all": false}`,
			expected:     "hi world\nhello again\nhello world",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.setupFile != "" {
				backend.Write(ctx, tt.setupFile, tt.setupContent)
			}
			...
			result, err := backend.Read(ctx, tt.setupFile, 0, 0)
			if err != nil {
				t.Fatalf("edit_file tool failed: %v", err)
			}
			if result != tt.expected {
