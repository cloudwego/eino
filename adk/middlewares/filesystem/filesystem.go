...
type MiddlewareConfig struct {
	...
	// EnableMultiSandboxBackend enables multi-sandbox routing on a single Backend implementation.
	//
	// When enabled:
	//   - All filesystem tools (ls, read_file, write_file, edit_file, glob, grep) and the execute tool
	//     gain an extra "sandbox_name" parameter in their JSON schema, exposed to the model.
	//   - The model chooses which sandbox to operate on per tool call by setting "sandbox_name".
	//   - Before dispatching to Backend / Shell / StreamingShell, the middleware injects the chosen
	//     sandbox name into context via filesystem.WithSandboxName.
	//   - Backend/Shell/StreamingShell implementations retrieve it via filesystem.SandboxFromContext(ctx)
	//     and route the operation to the corresponding sandbox.
	//
	// The Backend implementation is responsible for interpreting sandbox names
	// and dispatching accordingly. If disabled (default), tool schemas are unchanged
	// and no sandbox name appears in context.
	//
	// optional, false by default
	EnableMultiSandboxBackend bool
...
func getFilesystemTools(_ context.Context, middlewareConfig *MiddlewareConfig) ([]tool.BaseTool, error) {
	...
	toolSpecs := []toolSpec{
		{
			...
			createFunc: func(name, desc string) (tool.BaseTool, error) {
				if middlewareConfig.Backend != nil {
					if middlewareConfig.EnableMultiSandboxBackend {
						return newSandboxLsTool(middlewareConfig.Backend, name, desc)
					}
			...
			createFunc: func(name, desc string) (tool.BaseTool, error) {
				if middlewareConfig.Backend != nil {
					if middlewareConfig.UseMultiModalRead {
						if middlewareConfig.EnableMultiSandboxBackend {
							return newSandboxMultiModalReadFileTool(middlewareConfig.Backend, name, desc)
						}
						return newMultiModalReadFileTool(middlewareConfig.Backend, name, desc)
					}
					if middlewareConfig.EnableMultiSandboxBackend {
						return newSandboxReadFileTool(middlewareConfig.Backend, name, desc)
					}
		...
		},
		{
			...
			createFunc: func(name, desc string) (tool.BaseTool, error) {
				if middlewareConfig.Backend != nil {
					if middlewareConfig.EnableMultiSandboxBackend {
						return newSandboxWriteFileTool(middlewareConfig.Backend, name, desc)
					}
		...
		},
		{
			...
			createFunc: func(name, desc string) (tool.BaseTool, error) {
				if middlewareConfig.Backend != nil {
					if middlewareConfig.EnableMultiSandboxBackend {
						return newSandboxEditFileTool(middlewareConfig.Backend, name, desc)
					}
		...
		},
		{
			...
			createFunc: func(name, desc string) (tool.BaseTool, error) {
				if middlewareConfig.Backend != nil {
					if middlewareConfig.EnableMultiSandboxBackend {
						return newSandboxGlobTool(middlewareConfig.Backend, name, desc)
					}
		...
		},
		{
			...
			createFunc: func(name, desc string) (tool.BaseTool, error) {
				if middlewareConfig.Backend != nil {
					if middlewareConfig.EnableMultiSandboxBackend {
						return newSandboxGrepTool(middlewareConfig.Backend, name, desc)
					}
	...
	if middlewareConfig.StreamingShell != nil {
		...
		if middlewareConfig.EnableMultiSandboxBackend {
			executeTool, err = newSandboxStreamingExecuteTool(middlewareConfig.StreamingShell, ToolNameExecute, executeDesc)
		} else {
		...
		var executeTool tool.BaseTool
		if middlewareConfig.EnableMultiSandboxBackend {
			executeTool, err = newSandboxExecuteTool(middlewareConfig.Shell, ToolNameExecute, executeDesc)
		} else {
...
func newReadFileTool(fs filesystem.Backend, name string, desc string) (tool.BaseTool, error) {
	...
	return utils.InferTool(toolName, d, func(ctx context.Context, input readFileArgs) (string, error) {
		return readFileHandler(ctx, fs, input)
	})
}
...
func newMultiModalReadFileTool(fs filesystem.Backend, name string, desc string) (tool.BaseTool, error) {
	...
	return utils.InferEnhancedTool(toolName, d, func(ctx context.Context, input multiModalReadFileArgs) (*schema.ToolResult, error) {
		return multiModalReadFileHandler(ctx, er, input)
	})
}
...
func newWriteFileTool(fs filesystem.Backend, name string, desc string) (tool.BaseTool, error) {
	...
	return utils.InferTool(toolName, d, func(ctx context.Context, input writeFileArgs) (string, error) {
		return writeFileHandler(ctx, fs, input)
	})
}
...
func newEditFileTool(fs filesystem.Backend, name string, desc string) (tool.BaseTool, error) {
	...
	return utils.InferTool(toolName, d, func(ctx context.Context, input editFileArgs) (string, error) {
		return editFileHandler(ctx, fs, input)
	})
}
...
func newGlobTool(fs filesystem.Backend, name string, desc string) (tool.BaseTool, error) {
	...
	return utils.InferTool(toolName, d, func(ctx context.Context, input globArgs) (string, error) {
		return globHandler(ctx, fs, input)
	})
}
...
func newGrepTool(fs filesystem.Backend, name string, desc string) (tool.BaseTool, error) {
	...
	return utils.InferTool(toolName, d, func(ctx context.Context, input grepArgs) (string, error) {
		return grepHandler(ctx, fs, input)
	})
}
...
func newExecuteTool(sb filesystem.Shell, name string, desc string) (tool.BaseTool, error) {
	...
	return utils.InferTool(toolName, d, func(ctx context.Context, input executeArgs) (string, error) {
		return executeHandler(ctx, sb, input)
	})
}
...
func newStreamingExecuteTool(sb filesystem.StreamingShell, name string, desc string) (tool.BaseTool, error) {
	...
	return utils.InferStreamTool(toolName, d, func(ctx context.Context, input executeArgs) (*schema.StreamReader[string], error) {
		return streamingExecuteHandler(ctx, sb, input)
	})
}
