...
// Package utils provides utility builders for the summarization middleware.
package utils
...
type FinalizerBuilder struct {
	handlers []summarization.FinalizeFunc
	custom   summarization.FinalizeFunc
	errs     []error
}

// NewFinalizer creates a new FinalizerBuilder that builds a summarization.FinalizeFunc
...
// Example:
//
//	finalizer, err := utils.NewFinalizer().
//		PreserveSkills(&utils.PreserveSkillsConfig{}).
...
func (b *FinalizerBuilder) Custom(fn summarization.FinalizeFunc) *FinalizerBuilder {
	b.custom = fn
	return b
}

func (b *FinalizerBuilder) Build() (summarization.FinalizeFunc, error) {
	if len(b.errs) > 0 {
	...
	handlers := make([]summarization.FinalizeFunc, len(b.handlers))
...
func (b *FinalizerBuilder) PreserveSkills(config *PreserveSkillsConfig) *FinalizerBuilder {
	...
	b.handlers = append(b.handlers, func(ctx context.Context, originalMessages []adk.Message, summary adk.Message) ([]adk.Message, error) {
		summarizeInputMsgs, _ := ctx.Value(internal.CtxKeySummarizeInput{}).([]adk.Message)
...
func buildPreservedSkillsText(_ context.Context, messages []adk.Message, config *PreserveSkillsConfig) (string, error) {
	...
	for _, skill := range skills {
		parts = append(parts, fmt.Sprintf(internal.SkillSectionFormat, skill.Name, skill.Content))
	}

	skillsText := strings.Join(parts, "\n\n---\n\n")
	skillsText = fmt.Sprintf(internal.GetSkillPreamble(), skillsText)
