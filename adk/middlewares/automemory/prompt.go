...
func buildSystemMemoryInstruction(baseInstruction, marker, memoryInstruction string, stores []memoryStorePromptInfo) (string, error) {
...
func buildMemoryStoresManifestEnglish(stores []memoryStorePromptInfo) string {
	...
	for i, store := range stores {
		lines = append(lines,
			fmt.Sprintf("## %d. %s", i+1, store.Name),
			fmt.Sprintf("Mount: %s", store.Mount),
		)
		if strings.TrimSpace(store.Description) != "" {
			lines = append(lines, fmt.Sprintf("Purpose: %s", strings.TrimSpace(store.Description)))
		}
		if store.Index != nil {
			lines = append(lines, fmt.Sprintf("Index: %s", store.Index.Path), "")
			lines = append(lines, buildMemoryIndexBlockEnglish(*store.Index))
		}
		lines = append(lines, "")
	}
...
func buildMemoryStoresManifestChinese(stores []memoryStorePromptInfo) string {
	...
	for i, store := range stores {
		lines = append(lines,
			fmt.Sprintf("## %d. %s", i+1, store.Name),
			fmt.Sprintf("挂载路径：%s", store.Mount),
		)
		if strings.TrimSpace(store.Description) != "" {
			lines = append(lines, fmt.Sprintf("用途：%s", strings.TrimSpace(store.Description)))
		}
		if store.Index != nil {
			lines = append(lines, fmt.Sprintf("索引：%s", store.Index.Path), "")
			lines = append(lines, buildMemoryIndexBlockChinese(*store.Index))
		}
		lines = append(lines, "")
	}
	return strings.Join(lines, "\n")
...
func buildMemoryIndexBlockEnglish(index memoryIndexPromptInfo) string {
	lines := []string{fmt.Sprintf("### Index content: %s", index.FileName)}
	if index.Empty {
...
func buildMemoryIndexBlockChinese(index memoryIndexPromptInfo) string {
	lines := []string{fmt.Sprintf("### 索引内容：%s", index.FileName)}
	if index.Empty {
		lines = append(lines, getAppendEmptyIndexTemplate())
	} else {
		lines = append(lines, index.Content)
...
func buildExtractAutoOnlyPrompt(memoryStores string, newMessageCount int, existingMemories string, skipIndex bool) string {
	return internal.SelectPrompt(internal.I18nPrompts{
		English: buildExtractAutoOnlyPromptEnglish(memoryStores, newMessageCount, existingMemories, skipIndex),
		Chinese: buildExtractAutoOnlyPromptChinese(memoryStores, newMessageCount, existingMemories, skipIndex),
	})
}

func buildExtractionMemoryStores(stores []memoryStorePromptInfo) string {
	return internal.SelectPrompt(internal.I18nPrompts{
		English: buildExtractionMemoryStoresEnglish(stores),
		Chinese: buildExtractionMemoryStoresChinese(stores),
	})
}

func buildExtractionMemoryStoresEnglish(stores []memoryStorePromptInfo) string {
	lines := make([]string, 0, len(stores)*4)
	for _, store := range stores {
		lines = append(lines,
			fmt.Sprintf("- %s", store.Name),
			fmt.Sprintf("  Absolute path: %s", store.Mount),
			fmt.Sprintf("  Relative path prefix: %s/", store.Name),
		)
		if strings.TrimSpace(store.Description) != "" {
...
func buildExtractionMemoryStoresChinese(stores []memoryStorePromptInfo) string {
	lines := make([]string, 0, len(stores)*4)
	for _, store := range stores {
		lines = append(lines,
			fmt.Sprintf("- %s", store.Name),
			fmt.Sprintf("  绝对路径：%s", store.Mount),
			fmt.Sprintf("  相对路径前缀：%s/", store.Name),
		)
		if strings.TrimSpace(store.Description) != "" {
			lines = append(lines, fmt.Sprintf("  描述：%s", strings.TrimSpace(store.Description)))
		}
	}
	return strings.Join(lines, "\n")
}
...
func buildExtractAutoOnlyPromptEnglish(memoryStores string, newMessageCount int, existingMemories string, skipIndex bool) string {
	manifest := ""
	if existingMemories != "" {
...
func buildExtractAutoOnlyPromptChinese(memoryStores string, newMessageCount int, existingMemories string, skipIndex bool) string {
	manifest := ""
	if existingMemories != "" {
