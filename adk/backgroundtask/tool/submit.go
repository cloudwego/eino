...
func Submit(
	...
	if !ok {
		return nil, fmt.Errorf(
			"backgroundtask/tool: tool %q is not registered",
			req.ToolName,
		)
	}
	...
	if taskID == "" {
		taskID, err = manager.AllocateTaskID(
			ctx,
			&backgroundtask.AllocateTaskIDRequest{Kind: "background_tool"},
		)
		if err != nil {
			return nil, err
		}
	...
	if registration.Materializer != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("backgroundtask/tool: reserve output: %w", err)
		}
		if outputFile == "" {
			return nil, errors.New(
				"backgroundtask/tool: output materializer returned an empty path",
			)
		}
	}
...
func buildTaskSpec(
	...
	if err != nil {
		return backgroundtask.Spec{}, fmt.Errorf(
			"backgroundtask/tool: encode payload: %w",
			err,
		)
	}
