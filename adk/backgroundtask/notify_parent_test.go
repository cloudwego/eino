	...
	executor := &scriptedExecutor{
		execute: func(
			...
			case 1:
				...
				return &ExecutionResult{Directive: ExecutionDirectiveYield}, nil
			case 2:
				require.NoError(t, NotifyParent(ctx, &NotifyParentRequest{
				...
				return &ExecutionResult{
					Status: StatusCompleted, Data: []byte("done"),
				}, nil
			default:
...
func TestManagerNotifyParentUnavailableWithoutStoreCapability_BitsUT(t *testing.T) {
	var notifyErr error
	executor := &scriptedExecutor{
		execute: func(
			ctx context.Context,
			...
			notifyErr = NotifyParent(ctx, &NotifyParentRequest{
	...
	require.NoError(t, err)
	require.NoError(t, manager.Execute(context.Background(), task.Spec.ID))
	require.ErrorIs(t, notifyErr, ErrNotificationUnavailable)
}
