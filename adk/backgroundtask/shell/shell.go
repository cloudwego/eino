...
func NewRegistration(config *RegistrationConfig) (*backgroundtool.Registration, error) {
	if config == nil || config.Info == nil || config.Shell == nil {
		return nil, errors.New("backgroundtask/shell: tool info and recoverable shell are required")
	}
...
func (a *adapter) Recover(
	...
	if request == nil {
		return nil, errors.New("backgroundtask/shell: recover request is required")
	}
	input, err := decodeArguments(request.Arguments)
	if err != nil {
		return nil, err
...
func decodeArguments(value string) (*arguments, error) {
	...
	if input.Command == "" {
		return nil, errors.New("backgroundtask/shell: command is required")
	}
	return &input, nil
}
