...
type Plan interface {
	StepList(ctx context.Context) []any
	Unmarshal(ctx context.Context, arguments string) error
}

// plan the default implementation of Plan.
//
// JSON Schema:
//
//	{
...
func (p *plan) StepList(ctx context.Context) []any {
	stepList := make([]any, 0, len(p.Steps))
...
func (p *plan) Unmarshal(ctx context.Context, arguments string) error {
...
- Adaptability: Can the plan handle unexpected results or changes?`
	...
	ExecuteResultSessionKey = "ExecutedStep"
...
func formatPlan(ctx context.Context, p Plan) string {
	...
	for i, step := range p.StepList(ctx) {
...
func NewExecutor(ctx context.Context, cfg *ExecutorConfig) (adk.Agent, error) {
	...
	genInput := func(ctx context.Context, instruction string, _ *adk.AgentInput) ([]adk.Message, error) {
		...
		executedStep, ok := adk.GetSessionValue(ctx, ExecuteResultsSessionKey)
		if ok {
...
func (r *replanner) genInput(ctx context.Context) ([]adk.Message, error) {
	...
	plan_ := plan.(Plan)
	step := plan_.StepList(ctx)[0]

	var executedSteps_ []ExecutedStep
	executedSteps, ok := adk.GetSessionValue(ctx, ExecuteResultsSessionKey)
	if ok {
	...
	adk.SetSessionValue(ctx, ExecuteResultsSessionKey, executedSteps_)
...
func buildDefaultReplannerInputFn(in *PlanExecuteInput, planToolName, respondToolName string) GenPlanExecuteInputFn {
	return func(ctx context.Context, in *PlanExecuteInput) ([]adk.Message, error) {
		msgs, err := ReplannerUserPrompt.Format(ctx, map[string]any{
			"plan":             formatPlan(ctx, in.Plan),
			"input":            formatInput(in.Input),
			"executed_results": formatExecutedSteps(in.ExecutedSteps),
