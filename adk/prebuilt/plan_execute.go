...
// the instance of Plan has two methods:
//   - Steps: get the list of steps. the step will be rendered into the model prompt.
//   - Unmarshal: used for JSON deserialization of the plan output generated by the model.
type Plan interface {
	Steps(ctx context.Context) []Step
	Unmarshal(ctx context.Context, arguments string) error
}
...
func (s executionStep) Description(ctx context.Context) string {
	return string(s)
}

func (p *executionPlan) Steps(ctx context.Context) []Step {
	stepList := make([]Step, 0, len(p.Steps_))
	for _, step := range p.Steps_ {
		stepList = append(stepList, executionStep(step))
	}
	return stepList
}

func (p *executionPlan) Unmarshal(ctx context.Context, arguments string) error {
	return sonic.UnmarshalString(arguments, p)
}
...
type NewPlanFn func(ctx context.Context) Plan

func defaultNewPlanFn(ctx context.Context) Plan {
...
func (p *planner) Run(ctx context.Context, input *adk.AgentInput,
	...
	go func() {
		...
		plan_ := p.newPlanFn(ctx)
		err = plan_.Unmarshal(ctx, planJSON)
		if err != nil {
...
func NewPlanner(_ context.Context, cfg *PlannerConfig) (adk.Agent, error) {
	...
	newPlanFn := cfg.NewPlanFn
	if newPlanFn == nil {
		newPlanFn = defaultNewPlanFn
	}
...
func formatPlan(ctx context.Context, p Plan) string {
	var formattedPlan strings.Builder
	for i, step := range p.Steps(ctx) {
		formattedPlan.WriteString(fmt.Sprintf("%d. %s\n", i+1, step.Description(ctx)))
	}
...
func (r *replanner) genInput(ctx context.Context) ([]adk.Message, error) {
	...
	executeStep_ := executedStep.(string)
...
func (r *replanner) Run(ctx context.Context, input *adk.AgentInput, _ ...adk.AgentRunOption) *adk.AsyncIterator[*adk.AgentEvent] {
	...
	go func() {
		...
		plan_ := r.newPlanFn(ctx)
		err = plan_.Unmarshal(ctx, planMsg.ToolCalls[0].Function.Arguments)
		if err != nil {
			err = fmt.Errorf("unmarshal plan error: %w", err)
			generator.Send(&adk.AgentEvent{Err: err})
			return
