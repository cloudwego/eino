...
func TestSupervisorWithPlanExecuteInterruptResume(t *testing.T) {
	...
	checkpointID := "test-supervisor-planexecute-1"
...
func TestNestedSupervisorInterruptResume(t *testing.T) {
	...
	innerSupervisorResumable, ok := innerSupervisorAgent.(adk.ResumableAgent)
	assert.True(t, ok, "supervisor.New should return a ResumableAgent")

	innerSupervisorWrapped := &namedAgent{
		ResumableAgent: innerSupervisorResumable,
	...
	store := newIntegrationCheckpointStore()
	runner := adk.NewRunner(ctx, adk.RunnerConfig{
		Agent:           outerSupervisorAgent,
		CheckPointStore: store,
	})

	t.Log("========================================")
	t.Log("Starting Nested Supervisor Integration Test")
	t.Log("Hierarchy: company_coordinator -> payment_department -> payment_supervisor -> payment_worker -> process_payment tool")
	t.Log("========================================")
	...
	t.Logf("Interrupt event received with %d contexts", len(interruptEvent.Action.Interrupted.InterruptContexts))
	for i, intCtx := range interruptEvent.Action.Interrupted.InterruptContexts {
		t.Logf("Interrupt context %d: ID=%s, Info=%v, IsRootCause=%v", i, intCtx.ID, intCtx.Info, intCtx.IsRootCause)
	}
	...
	for {
		...
		resumeEvents = append(resumeEvents, event)
		t.Logf("Resume Event: %s", formatAgentEventIntegration(event))
	}
