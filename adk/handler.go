...
var baseChatModelAgentMiddlewareType = reflect.TypeOf(&BaseChatModelAgentMiddleware{})
...
func isMethodOverridden(handler ChatModelAgentMiddleware, methodName string) bool {
	...
	baseMethod, ok2 := baseChatModelAgentMiddlewareType.MethodByName(methodName)
	if !ok1 || !ok2 {
		return true
	}
