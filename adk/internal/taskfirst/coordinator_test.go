...
type controlledExecutor struct {
	...
	once           sync.Once
	ignoreControls bool
	acknowledge    func(*background.TaskSnapshot) error
...
func (e *controlledExecutor) AcknowledgeCancellation(
	_ context.Context,
...
type publishErrorStore struct {
	*background.InMemoryStore
	err error
}

func (s *publishErrorStore) Publish(
	context.Context,
...
func newControlledExecution(
	...
	execution, manager := newControlledExecutionWithStore(
		t, store, executor, policy,
	)
	return execution, manager, executor
}

func newControlledExecutionWithStore(
	t *testing.T,
...
func TestExecutionBoundaryAndTimeoutChannelsAreStable(t *testing.T) {
	...
	require.Equal(t, boundary, execution.Boundary())
	for i := 0; i < 100; i++ {
		require.Equal(t, timeout, execution.Timeout())
	}
	select {
...
func TestExecutionWaitingInputIsForegroundBoundary(t *testing.T) {
	...
	execution, _ := newControlledExecutionWithStore(
		t,
		background.NewInMemoryStore(nil),
		executor,
		&Policy{},
	)
	...
	case <-time.After(time.Second):
		t.Fatal("waiting-input did not close the foreground boundary")
	}
	boundary, err := execution.WaitBoundary(context.Background())
	require.NoError(t, err)
	require.Equal(t, background.StatusWaitingInput, boundary.Status)
	require.Equal(t, []byte("waiting"), boundary.Checkpoint)
}
...
func TestAwaitPublishesOnTimeoutAndLeavesTaskRunning(t *testing.T) {
	...
	close(executor.release)
	terminal, err := execution.WaitBoundary(context.Background())
	require.NoError(t, err)
...
func TestExecutionObservesAttemptClaimedByAnotherManager(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, background.StatusCompleted, terminal.Status)
	for index, manager := range []*background.Manager{managerOne, managerTwo} {
		...
		if index == 0 {
			require.True(
				t,
				closeErr == nil || errors.Is(closeErr, background.ErrVersionConflict),
			)
			continue
		}
...
func TestAwaitRejectedTimeoutWaitsForCanceledTerminal(t *testing.T) {
	t.Run("cooperative executor returns canceled terminal", func(t *testing.T) {
	...
	t.Run("blocked executor returns no partial outcome", func(t *testing.T) {
		store := background.NewInMemoryStore(nil)
		executor := &controlledExecutor{
			started:        make(chan struct{}),
			release:        make(chan struct{}),
		...
		stored, getErr := manager.Get(context.Background(), execution.TaskID())
		require.NoError(t, getErr)
		require.Equal(t, background.StatusRunning, stored.Status)
...
func TestAwaitCallerAbortPolicy(t *testing.T) {
	...
	t.Run("cleanup failure returns no partial outcome", func(t *testing.T) {
		hookErr := errors.New("cleanup hook failed")
		executor := &controlledExecutor{
			started: make(chan struct{}),
			release: make(chan struct{}),
			acknowledge: func(*background.TaskSnapshot) error {
...
func TestPublishFailureCleanupReportsHookFailureAfterPersistingIntent(t *testing.T) {
	publishErr := errors.New("publish failed")
	hookErr := errors.New("cleanup hook failed")
	store := &publishErrorStore{
		InMemoryStore: background.NewInMemoryStore(nil),
		err:           publishErr,
	}
	executor := &controlledExecutor{
		started: make(chan struct{}),
		release: make(chan struct{}),
		acknowledge: func(task *background.TaskSnapshot) error {
	...
	execution, manager := newControlledExecutionWithStore(
		...
		store,
		executor,
		&Policy{CleanupTimeout: time.Second},
	)
	...
	require.ErrorIs(t, err, hookErr)
	require.ErrorContains(t, err, "cancellation cleanup failed")
	require.NotNil(t, outcome)
	require.NotNil(t, outcome.Task)
	require.NotNil(t, outcome.Task.CancelRequestedAt)

	stored, getErr := manager.Get(context.Background(), execution.TaskID())
	require.NoError(t, getErr)
	require.Equal(t, background.PublicationDeferred, stored.Publication)
	require.NotNil(t, stored.CancelRequestedAt)
}

func TestPublishFailureCleanupIsBoundedForBlockedExecutor(t *testing.T) {
	publishErr := errors.New("publish failed")
	store := &publishErrorStore{
		InMemoryStore: background.NewInMemoryStore(nil),
		err:           publishErr,
	...
	execution, manager := newControlledExecutionWithStore(
		...
		store,
		executor,
		&Policy{CleanupTimeout: 20 * time.Millisecond},
	)
