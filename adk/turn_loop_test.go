...
type turnLoopCancellableMockAgent struct {
	name     string
	runFunc  func(ctx context.Context, input *AgentInput) (*AgentOutput, error)
	onCancel func(cc *cancelContext) // called when cancel is received via WithCancel
	mu       sync.Mutex
...
func (a *turnLoopCancellableMockAgent) Run(ctx context.Context, input *AgentInput, opts ...AgentRunOption) *AsyncIterator[*AgentEvent] {
	...
	localCancel := a.cancel
	a.mu.Unlock()

	// Extract cancelContext from opts and set up cancel propagation to context
	...
	if o.cancelCtx != nil {
		...
		go func() {
			...
			case <-cc.cancelChan:
				...
				a.mu.Unlock()
				// Mark as completed so the cancelFunc returns
				cc.markCompleted()
			case <-cancelCtx.Done():
			}
		}()
	...
	go func() {
		...
		_ = localCancel // prevent capture warning
		if err != nil {
			gen.Send(&AgentEvent{Err: err})
			return
		}
...
func TestTurnLoop_Preempt_WithAgentCancelMode(t *testing.T) {
	...
	agent := &turnLoopCancellableMockAgent{
		...
		onCancel: func(cc *cancelContext) {
			// Read the cancel config to see which mode was used
			cancelModeMu.Lock()
			if cc.config != nil {
				firstCancelModeUsed = cc.config.Mode
			}
			cancelReceivedOnce.Do(func() {
	...
	case <-cancelReceived:
	case <-time.After(1 * time.Second):
		t.Fatal("cancel was not received by agent")
	}
