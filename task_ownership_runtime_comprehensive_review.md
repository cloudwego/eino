...
| R6-09 | managed filesystem 的 timeout wire contract 曾在毫秒与秒之间分叉。 | 最新 `alpha/10` 已将公开 `timeout` 定义为秒，并通过结构化 `ForegroundTimeoutError` 报告失败。 | 重新引入 `timeout_seconds` 与 legacy 毫秒 `timeout` 会制造双字段和重放歧义。 | **Fix**：沿用公开 `timeout` 秒制，内部转换为毫秒并将正值上限 clamp 到 3 天；不引入 legacy 双字段，超时保留结构化错误契约。 |
